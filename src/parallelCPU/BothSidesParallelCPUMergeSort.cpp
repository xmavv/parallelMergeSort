#include "BothSidesParallelCPUMergeSort.h"

#include <thread>

BothSidesParallelCPUMergeSort::BothSidesParallelCPUMergeSort(vector<int>& list, int parallel_threshold, int max_depth)
{
    set_list(list);
...
void BothSidesParallelCPUMergeSort::set_list(vector<int>& list)
{
    this->list = list;
}

void BothSidesParallelCPUMergeSort::set_parallel_threshold(int parallel_threshold)
{
    this->parallel_threshold = parallel_threshold;
}

void BothSidesParallelCPUMergeSort::set_max_depth(int max_depth)
{
    this->max_depth = max_depth;
}

vector<int> BothSidesParallelCPUMergeSort::run(int left, int right, int depth)
{
    sort_parallel(left, right, depth);
    return this->list;
}

void BothSidesParallelCPUMergeSort::sort_parallel(int left, int right, int depth)
{
    ...
    thread right_thread([=, this]() {
        this->sort_parallel(mid + 1, right, depth + 1);
    });
...
void BothSidesParallelCPUMergeSort::sort_sequential(int left, int right)
{
    if (left >= right) return;
...
void BothSidesParallelCPUMergeSort::merge(int left, int mid, int right)
{
